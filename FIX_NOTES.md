...
### v20: 移除 pre-pair unBond, 强制开启 discovery (用户要求)
...
用户配对 PC2310MLJB250506G 成功进入槽位 1, 但连接 3-4 秒后断链:
...
### v22: 3 点模式 override (用户选择 3 点仍要求配对 5 点)

根因: PICO 配置服务 (PxrConfigService) 拒绝应用写入系统键,
`sys_tracking_tracker_wear_mode` 卡在 "3"(5tk_thigh),
`com.pvr.swift.upper.limit` 卡在 5, 用户在 UI 里选择 3 点无法落盘
...
之后用户在应用内切换 3 点/5 点 (穿戴模式页/设置数量上限) 均会即时生效。

当前产物: swift205_patched_v22.apk, md5 0e0e36c87f5b97f1601e2cecd038e26a

### v23: 穿戴模式切换联动配对上限 (用户要求 2/3/5 点切换可用)
...
工作, 5 点限制大概率是产品档位软件门 (已绕过), 但用户只有 3 台追踪器,
